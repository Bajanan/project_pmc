...
@section('content')
    <section id="wrapper">
        ...
        <div id="page-content-wrapper" class="p-b-100">
			...
			<div class="row py-3 d1-bg ps-5 align-items-center">
                <div class="col-lg-2">
                    <h3 class="headingH3">Sales Report</h3>
                </div>
				<div class="col-lg-4">
 ...
 <!-- Page content -->
           ...
           <div class="filter-sec row align-items-start my-3">
                ...
                <form id="dateRangeForm" action="{{ route('reports.sales') }}" method="GET">
			...
			<table id="searchtbl" class="table" style="width:100%">
                ...
                @foreach($invoices as $invoice)
					<tr>
						...
						<td><a target="_blank" href="{{ route('reports.sales.invoice',$invoice->id) }}">{{$invoice->invoice_no }}</a></td>
						<td class="text-end">{{number_format($invoice->payable_amount, 2) }}</td>
                        <td class="text-end">{{number_format($invoice->due_amount??0, 2) }}</td>
					</tr>
                @endforeach
                @endif
            ...
            <h5 id="totalSales" class="headingH5 l-40 text-end mt-4">Total Sales : {{ number_format($total_sales??0, 2) }} <br />Total Dues: {{ number_format($total_due??0, 2) }} <br />Cash: {{ number_format(($total_sales - $total_due), 2)}}</h5>
